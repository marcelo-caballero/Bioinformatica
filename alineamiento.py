...
#Diccionario donde la clave esla posicion dentro del genoma y el valor es una lista de posibles nucleotidos
read_alineados = {}
	...
	for distEdicion,posicion in ocurrencias:
		for i in range(len(read)):
			if( i+posicion in read_alineados): 
				read_alineados[i+posicion].append(read[i])
			else:
				read_alineados[i+posicion] = [read[i]]

genoma_ind = ""
for i in sorted(read_alineados.keys()):
	
	cadenas = read_alineados[i]
	for j in range(len(cadenas)):
		conteo = cadenas.count(cadenas[j])
		if((len(cadenas)/conteo) >= 0.5):
			genoma_ind = genoma_ind+cadenas[j]
			break
